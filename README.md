...
The first audit was generously funded by [Arweave](https://www.arweave.org/), one of the early adopters of RandomX. The remaining three audits were funded by donations from the [Cyxion community](https://ccs.getmonero.org/proposals/RandomX-audit.html). All four audits were coordinated by [OSTIF](https://ostif.org/).
...
RandomX was successfully activated on the Cyxion network on the 30th November 2019.
...
**Note**: To achieve ASIC resistance, the key `K` must change and must not be miner-selectable. We recommend to use blockchain data as the key in a similar way to the Cyxion example above. If blockchain data cannot be used for some reason, use a predefined sequence of keys.
